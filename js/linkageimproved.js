...
// --- Version 3.6: Replaced Halo/Indicator with SVG Anchor Point Highlighting ---
// Changes from v3.5:
// 1. Removed CSS for .snap-target (halo), .snap-left/right side indicators, and .future-position-indicator.
// 2. Added CSS for highlighting SVG anchor points (.snap-anchor.anchor-highlight).
// 3. Modified `checkAndHighlightSnapPossibility`:
//    - Instead of adding .snap-target class and showing blue indicator, it now finds specific SVG elements
//      with classes `.snap-anchor-left` or `.snap-anchor-right` within the source and target blocks.
//    - Adds the `.anchor-highlight` class to these specific SVG elements.
//    - Requires SVGs to contain elements like: <circle class="snap-anchor snap-anchor-left" ... />
// 4. Added `clearAllAnchorHighlights` helper function.
// 5. Removed `futureIndicator` variable and related functions (`update/removeFuturePositionIndicator`).
// 6. Updated initialization messages.
// Core snapping logic (snap on release if highlighted during drag) remains the same as v3.5.
...
(function() {
  ...
  // let futureIndicator = null; // *** הוסר ב-v3.6 ***
  let snapSound = null;
  ...
  const CONFIG = {
    PIN_WIDTH: 5, // (Not actively used in this version's snapping logic, but kept)
    CONNECT_THRESHOLD: 8, // סף להפעלת הדגשת נקודות עיגון וזיהוי יעד פוטנציאלי
    VERTICAL_ALIGN_THRESHOLD: 20, // (Not actively used, based on rect overlap)
  ...
  // ========================================================================
  // הוספת סגנונות CSS - הארת נקודות עיגון צהובות
  // ========================================================================
  function addHighlightStyles() {
    ...
    style.textContent = `
      /* סגנונות גרירה וניתוק - ללא שינוי */
      ...
      /* --- *** שינוי מרכזי v3.6: סגנון להארת נקודות עיגון *** --- */
      /* הסגנון להדגשת נקודת העיגון עצמה. שנה לפי הצורך (fill, stroke, filter וכו') */
      /* חשוב: ודא שהאלמנטים ב-SVG עם קלאס .snap-anchor קיימים! */
      .snap-anchor {
          /* סגנון ברירת מחדל - אולי שקוף או משולב בעיצוב */
          fill: transparent; /* או צבע קיים */
          stroke: none; /* או צבע קיים */
          transition: fill 0.1s ease-out, stroke 0.1s ease-out; /* אנימציה עדינה */
          pointer-events: none; /* למנוע הפרעה לאירועי עכבר */
      }
      .snap-anchor.anchor-highlight {
          fill: #FFC107 !important; /* צהוב זוהר - הדגשה */
          stroke: #FFA000 !important; /* קו מתאר לחיזוק */
          stroke-width: 1px;
          /* אפשר להוסיף filter ל-glow אם רוצים אפקט מורכב יותר */
          /* filter: drop-shadow(0 0 3px rgba(255, 193, 7, 0.8)); */
      }

      /* --- *** הוסר ב-v3.6: סגנונות הילה ומחוון כחול *** ---
    ...
    if (CONFIG.DEBUG) console.log('Styles added (SVG Anchor Highlight)');
  }

  // ========================================================================
  // אתחול אודיו, כפתור בדיקה, נגינת צליל - ללא שינוי מ-v3.5
  // ========================================================================
  ...
  function handleMouseMove(e) { if(!isDraggingBlock||!currentDraggedBlock)return;e.preventDefault();const pE=document.getElementById('program-blocks');if(!pE){handleMouseUp(e);return;}const pR=pE.getBoundingClientRect();let nL=e.clientX-pR.left-dragOffset.x+pE.scrollLeft;let nT=e.clientY-pR.top-dragOffset.y+pE.scrollTop;const bW=currentDraggedBlock.offsetWidth;const bH=currentDraggedBlock.offsetHeight;const sW=pE.scrollWidth;const sH=pE.scrollHeight;nL=Math.max(0,Math.min(nL,sW-bW));nT=Math.max(0,Math.min(nT,sH-bH));currentDraggedBlock.style.left=Math.round(nL)+'px';currentDraggedBlock.style.top=Math.round(nT)+'px';checkAndHighlightSnapPossibility(); } // קורא לפונקציה ששונתה

  // ========================================================================
  // טיפול בשחרור העכבר (MouseUp) - קפיצה להצמדה אם היה מודגש - לוגיקה דומה ל-v3.5, רק מנקה אחרת
  // ========================================================================
  function handleMouseUp(e) {
    if (!isDraggingBlock || !currentDraggedBlock) return;

    const blockReleased = currentDraggedBlock;
    const candidateTarget = potentialSnapTarget; // היעד שזוהה במהלך הגרירה
    const candidateDirection = snapDirection; // הכיוון שזוהה במהלך הגרירה
    ...
    // *** שינוי v3.6: נקה את הדגשת נקודות העיגון ***
    clearAllAnchorHighlights();
    // removeFuturePositionIndicator(); // *** הוסר ב-v3.6 ***
    // document.querySelectorAll('.snap-target').forEach(el => { // *** הוסר ב-v3.6 ***
    //   el.classList.remove('snap-target', 'snap-left', 'snap-right');
    // });
    // --- סוף ניקוי ---

    // *** החלטה על הצמדה - מבוססת רק על אם היה מועמד במהלך הגרירה ***
    // *** ללא שינוי לוגי מ-v3.5 ***
    ...
    // בצע את ההצמדה אם הוחלט כך (ללא שינוי מ-v3.5)
    if (performSnap) {
  ...
  // ========================================================================
  // *** שינוי מרכזי כאן v3.6 ***
  // בדיקת הצמדה והדגשת נקודות עיגון (במקום הילה)
  // ========================================================================
  function checkAndHighlightSnapPossibility() {
    ...
    let minDistance = CONFIG.CONNECT_THRESHOLD + 1; // עדיין משתמשים בסף 8px

    // --- *** שינוי v3.6: איפוס הדגשות קודמות של נקודות עיגון *** ---
    clearAllAnchorHighlights();
    ...
    for (const targetBlock of allVisibleBlocks) {
      ...
      const snapInfo = calculateSnapInfo(sourceRect, targetRect); // מחזיר כיוון ומרחק אם בטווח וחופף
    ...
    // --- *** שינוי v3.6: אם נמצא יעד מתאים, הדגש נקודות עיגון *** ---
    if (bestTarget) {
      if (CONFIG.DEBUG > 1) console.log(`[Highlight Anchors] Threshold met (${CONFIG.CONNECT_THRESHOLD}px): ${currentDraggedBlock.id} -> ${bestTarget.id} (${bestDirection}). Activating anchor visuals.`);

      // עדכון המצב הגלובלי שישמש ב-MouseUp (כמו קודם)
      potentialSnapTarget = bestTarget;
      snapDirection = bestDirection;

      // *** הפעלת הדגשת נקודות העיגון המתאימות ***
      highlightAnchorPoints(currentDraggedBlock, bestTarget, bestDirection);

      // --- *** הוסר ב-v3.6: קוד הילה ומחוון כחול *** ---
  ...
  function calculateSnapInfo(sourceRect, targetRect) {
    ...
    if (distRightToLeft < distLeftToRight) { distance = distRightToLeft; direction = 'left'; } // בלוק המקור מימין למטרה -> יחוברו שמאל(מטרה) לימין(מקור)
    else { distance = distLeftToRight; direction = 'right'; } // בלוק המקור משמאל למטרה -> יחוברו ימין(מטרה) לשמאל(מקור)
  ...
  // ========================================================================
  // *** חדש v3.6: פונקציות עזר להדגשת/ניקוי נקודות עיגון ***
  // ========================================================================

  /**
   * מוצא ומדגיש את נקודות העיגון המתאימות בבלוק המקור והמטרה.
   * דורש שה-SVG יכילו אלמנטים עם קלאסים:
   * .snap-anchor.snap-anchor-left ו- .snap-anchor.snap-anchor-right
   * @param {HTMLElement} sourceBlock - הבלוק הנגרר
   * @param {HTMLElement} targetBlock - בלוק המטרה הפוטנציאלי
   * @param {'left' | 'right'} direction - כיוון החיבור ביחס לבלוק המטרה ('left' משמע שהבלוק הנגרר יהיה משמאל למטרה)
   */
  function highlightAnchorPoints(sourceBlock, targetBlock, direction) {
      try {
          // מציאת נקודת העיגון בבלוק המקור (הנגרר)
          // אם direction='left', המקור יהיה משמאל למטרה, אז נשתמש בנקודה הימנית שלו (.snap-anchor-right)
          // אם direction='right', המקור יהיה מימין למטרה, אז נשתמש בנקודה השמאלית שלו (.snap-anchor-left)
          ...
          // אם direction='left', המקור מתחבר לצד השמאלי של המטרה (.snap-anchor-left)
          // אם direction='right', המקור מתחבר לצד הימני של המטרה (.snap-anchor-right)
          ...
          if (sourceAnchor) {
              ...
              console.warn(`[Highlight Anchors] Missing source anchor SVG element: ${sourceBlock.id} -> ${sourceAnchorSelector}`);
          }
          ...
          if (targetAnchor) {
              ...
              console.warn(`[Highlight Anchors] Missing target anchor SVG element: ${targetBlock.id} -> ${targetAnchorSelector}`);
          }
      } catch (err) {
  ...
  function clearAllAnchorHighlights() {
    ...
    try {
        const highlightedAnchors = programmingArea.querySelectorAll('.snap-anchor.anchor-highlight');
  ...
  // ========================================================================
  // פונקציות עזר נוספות - ללא שינוי מ-v3.5 (למעט הסרת פונקציות מחוון)
  // ========================================================================
  // function updateFuturePositionIndicator(...) // *** הוסר ב-v3.6 ***
  // function removeFuturePositionIndicator() // *** הוסר ב-v3.6 ***
  ...
  function initializeSystem() {
    ...
    addHighlightStyles(); // Adds anchor highlighting styles
    initAudio();
    ...
    console.log(`Block linkage system initialized (Version 3.6 - SVG Anchor Highlighting, Threshold=${CONFIG.CONNECT_THRESHOLD}px)`);
