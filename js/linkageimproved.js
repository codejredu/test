...
// --- Version 3.6: Reverted to embedded Base64 sound ---
...
(function() {
  ...
  const CONFIG = {
    ...
    VERTICAL_OVERLAP_REQ: 0.3,
    BLOCK_GAP: 0,
    PLAY_SOUND: true,
    SOUND_VOLUME: 0.8,
    // SOUND_PATH: 'assets/sound/link.mp3', // *** הוסר שימוש בקובץ חיצוני ***
    USE_EMBEDDED_SOUND: true, // *** דגל חדש לשליטה ***
    CONNECTED_SOURCE_ZINDEX: '10',
    CONNECTED_TARGET_ZINDEX: '9',
    DEBUG: true
  };

  // ========================================================================
  // הוספת סגנונות CSS (ללא שינוי מ-3.5)
  // ========================================================================
  function addHighlightStyles() {
    ...
    style.textContent = `
      ...
      .future-position-indicator { position: absolute; border: 3px dashed rgba(0,120,255,0.95); border-radius: 5px; background-color: rgba(0,120,255,0.15); pointer-events: none; z-index: 998; opacity: 0; transition: opacity 0.15s ease-out, left 0.05s linear, top 0.05s linear; display: none; }
      ...
      #detach-menu div { padding: 6px 12px; cursor: pointer; border-radius: 3px; } #detach-menu div:hover { background-color: #eee; }
      body.user-select-none { user-select: none; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; }
      .connected-block, .has-connected-block { position: absolute !important; }
      ...
      #sound-test-button:hover:not(:disabled) { background-color: #0b7dda; } #sound-test-button.success { background-color: #4CAF50; } #sound-test-button.error { background-color: #f44336; } #sound-test-button.loading { background-color: #ff9800; cursor: wait; } #sound-test-button.hidden { opacity: 0; pointer-events: none; }
    `;
  ...
  // ========================================================================
  // אתחול מערכת השמע - *** משתמש בצליל Base64 ***
  // ========================================================================
  function initAudio() {
    if (!CONFIG.PLAY_SOUND || !CONFIG.USE_EMBEDDED_SOUND) {
        if (CONFIG.DEBUG && !CONFIG.USE_EMBEDDED_SOUND) console.log("Embedded sound disabled by config.");
        soundInitialized = false; // Mark as not initialized if disabled
        snapSound = null;
        return;
    }
    if (soundInitialized && snapSound && document.body.contains(snapSound)) {
        if (CONFIG.DEBUG) console.log("Embedded audio already initialized.");
        return;
    }
    ...
    try {
      ...
      snapSound.preload = 'auto'; // Preload the embedded sound
      snapSound.volume = CONFIG.SOUND_VOLUME;

      // *** הוספת מקור Base64 (צליל קליק/הצמדה ברור) ***
      const source = document.createElement('source');
      // This is a short, distinct "click" sound (WAV format)
      source.src = 'data:audio/wav;base64,'; // Add the Base64 encoded WAV sound data here
      source.type = 'audio/wav'; // Set type to WAV
      snapSound.appendChild(source);

      // Fallback message if AudioContext is not available (optional but good practice)
      snapSound.innerHTML += 'Your browser does not support the audio element.';

      // Setup error handler for cases like unsupported format or file loading issues
      snapSound.addEventListener('error', (e) => {
          console.error('Error with audio element:', e.target.error);
          soundInitialized = false; // Mark as failed
          CONFIG.PLAY_SOUND = false; // Disable further attempts if setup fails
          snapSound = null;
      });

      // Setup 'canplaythrough' handler to confirm readiness
      snapSound.addEventListener('canplaythrough', () => {
        soundInitialized = true;
        if (CONFIG.DEBUG) console.log('Embedded audio initialized and ready to play.');
        // Update button if needed (logic moved below)
      }, { once: true }); // Use { once: true } to ensure it only runs once successfully

      snapSound.style.display = 'none';
      document.body.appendChild(snapSound);

      // Add test button immediately if using embedded sound
      if (CONFIG.PLAY_SOUND && CONFIG.USE_EMBEDDED_SOUND) {
        addSoundTestButton(); // Add button now, it will update if needed later
      }

      if (CONFIG.DEBUG) console.log('Embedded Audio element created.');

    } catch (err) {
      console.error('Error initializing embedded audio:', err);
      ...
      if (button) {
          button.textContent = 'שגיאה באתחול שמע'; button.classList.add('error'); button.disabled = true; button.style.backgroundColor = '#f44336';
      }
    }
  }

  // ========================================================================
  // הוספת כפתור בדיקת שמע - מותאם ל-Base64
  // ========================================================================
  function addSoundTestButton() {
    // Only add if using embedded sound OR if external sound is configured (and check its state)
    if (!CONFIG.PLAY_SOUND || (!CONFIG.USE_EMBEDDED_SOUND /* && !CONFIG.SOUND_PATH - Removed this part as we only handle embedded now */)) {
        return;
    }
    if (!CONFIG.USE_EMBEDDED_SOUND){
        // Logic for external sound button remains the same as v3.5 - for now this branch is unused
         console.warn("External sound test button logic not active in this version (USE_EMBEDDED_SOUND=true).")
         return;
    }

    // Logic for Embedded Sound Button
    try {
      ...
      button.title = 'בדוק צליל הצמדה (מובנה)';
      button.className = '';

      // Embedded sound is usually ready immediately or fails quickly
      if (snapSound) { // If snapSound was created successfully in initAudio
          button.textContent = 'בדוק צליל הצמדה';
          button.style.backgroundColor = '#2196F3';
          button.disabled = false;
          soundInitialized = true; // Assume ready if element exists for Base64
      } else {
          button.textContent = 'שגיאה בשמע מובנה';
          button.classList.add('error');
          button.style.backgroundColor = '#f44336';
          button.disabled = true;
          soundInitialized = false; // Ensure it's marked as failed
      }
      ...
      // Hover effect
      button.onmouseover = function() { if (!this.disabled && !this.classList.contains('success')) this.style.backgroundColor = '#0b7dda'; };
      button.onmouseout = function() { if (!this.disabled && !this.classList.contains('success')) this.style.backgroundColor = '#2196F3'; };

      // Click listener
      button.addEventListener('click', function() {
        if (this.disabled || !snapSound) { console.warn('Sound test clicked but audio not ready.'); return; }

        snapSound.play().then(() => {
          if (CONFIG.DEBUG) console.log('Embedded sound test successful!');
          button.textContent = 'צליל מובנה תקין ✓';
          button.classList.remove('error'); button.classList.add('success'); button.style.backgroundColor = '#4CAF50';
          audioContextAllowed = true;
          setTimeout(() => { button.classList.add('hidden'); setTimeout(() => { if(button.parentNode) button.remove(); }, 500); }, 3000);
          if(snapSound) { snapSound.pause(); snapSound.currentTime = 0; }
        }).catch(err => {
          console.warn('Embedded sound test failed:', err);
          button.textContent = 'שמע חסום? לחץ שוב'; button.classList.add('error'); button.style.backgroundColor = '#f44336';
          audioContextAllowed = false;
        });
      });

      document.body.appendChild(button);
      if (CONFIG.DEBUG) console.log('Embedded sound test button added.');
  ...
  // ========================================================================
  // השמעת צליל הצמדה - מותאם ל-Base64
  // ========================================================================
  function playSnapSound() {
    // Check if sound is enabled AND embedded sound is specifically chosen AND initialized
    if (!CONFIG.PLAY_SOUND || !CONFIG.USE_EMBEDDED_SOUND || !snapSound || !soundInitialized) {
      if (CONFIG.DEBUG && CONFIG.PLAY_SOUND && CONFIG.USE_EMBEDDED_SOUND && (!snapSound || !soundInitialized)) {
          console.log(`Embedded snap sound skipped: (Sound Element: ${!!snapSound}, Initialized: ${soundInitialized})`);
      }
      return;
    }

    if (!audioContextAllowed && CONFIG.DEBUG) {
        console.warn('Attempting embedded sound before interaction.');
    }

    try {
      // With Base64, readyState is less critical but check anyway
      if (snapSound.readyState < (snapSound.HAVE_FUTURE_DATA || 3) ) { // Use constant or numerical value
          if (CONFIG.DEBUG) console.log('Embedded snap sound skipped: not enough data (readyState=' + snapSound.readyState + ').');
          return;
      }
      ...
      if (playPromise !== undefined) {
        playPromise.then(() => {
          audioContextAllowed = true;
          if (CONFIG.DEBUG > 1) console.log('Embedded snap sound played.');
        }).catch(err => {
          if (err.name === 'NotAllowedError') {
            console.warn('Embedded sound blocked by browser. User interaction needed.');
            audioContextAllowed = false;
            if (!document.getElementById('sound-test-button')) addSoundTestButton();
          } else if (err.name !== 'AbortError') { // Ignore AbortError
            console.error('Error playing embedded snap sound:', err);
          }
        });
      }
    } catch (err) {
      console.error('Unexpected error trying to play embedded snap sound:', err);
    }
  }
  ...
  // ========================================================================
  // טיפול בלחיצת עכבר על בלוק (ללא שינוי)
  // ========================================================================
  function handleMouseDown(e) {
      ...
      } else { block.style.margin = '0'; }
      block.style.zIndex = '1001'; block.classList.add('snap-source'); document.body.classList.add('user-select-none');
  ...
  function handleContextMenu(e) {
      e.preventDefault(); const block = e.target.closest('.block-container');
  ...
  function handleMouseLeave(e) { if (isDraggingBlock && e.target === document.documentElement && !e.relatedTarget) { if (CONFIG.DEBUG) console.warn("Mouse left document during drag, treating as mouseup."); handleMouseUp(e); } }
  ...
  // ========================================================================
  // טיפול בשחרור עכבר (ללא שינוי מ-3.5)
  // ========================================================================
  function handleMouseUp(e) {
    if (!isDraggingBlock || !currentDraggedBlock) return;
    const blockReleased = currentDraggedBlock;
    const targetToSnap = potentialSnapTarget; const directionToSnap = snapDirection;
    ...
    document.body.classList.remove('user-select-none'); blockReleased.classList.remove('snap-source');
    // Z-INDEX NOT RESET HERE
    // Remove visual indicators
    document.querySelectorAll('.snap-target, .snap-left, .snap-right').forEach(el => el.classList.remove('snap-target', 'snap-left', 'snap-right'));
    removeFuturePositionIndicator();
    // Attempt snap
    if (targetToSnap && directionToSnap) {
      if (CONFIG.DEBUG) console.log(`[MouseUp] Attempting snap...`);
      snapSuccess = performBlockSnap(blockReleased, targetToSnap, directionToSnap);
    }
    // Final state if no snap or snap failed
    if (!snapSuccess) {
      blockReleased.draggable = true;
      blockReleased.style.zIndex = ''; // Reset z-index only if no snap
      if (CONFIG.DEBUG) console.log(`[MouseUp] No/failed snap. ${blockReleased.id} draggable=true, zIndex reset.`);
    }
    if (CONFIG.DEBUG) console.log(`[MouseUp] ----- End MouseUp for ${blockReleased.id} -----`);
  }

  // ========================================================================
  // בדיקת הצמדה והדגשה (ללא שינוי מ-3.5)
  // ========================================================================
  ...
  // ========================================================================
  // ביצוע ההצמדה (ללא שינוי מ-3.5)
  // ========================================================================
  function performBlockSnap(sourceBlock, targetBlock, direction) {
    if (!sourceBlock || !targetBlock || !document.body.contains(targetBlock) || targetBlock.offsetParent === null) { console.warn(`[PerformSnap] Invalid target ${targetBlock?.id}. Snap cancelled.`); return false; }
    const targetAttrToCheck = `data-connected-from-${direction}`;
    if (targetBlock.hasAttribute(targetAttrToCheck)) { console.warn(`[PerformSnap] Target ${targetBlock.id} slot '${direction}' occupied. Snap cancelled.`); return false; }
    ...
    try {
        ...
        if (targetStyle.position !== 'absolute') {
            ...
            targetBlock.style.position = 'absolute'; targetBlock.style.margin = '0';
        ...
        sourceBlock.style.position = 'absolute'; sourceBlock.style.margin = '0';
        sourceBlock.style.left = `${Math.round(styleLeft)}px`; sourceBlock.style.top = `${Math.round(styleTop)}px`;
        sourceBlock.style.zIndex = CONFIG.CONNECTED_SOURCE_ZINDEX; targetBlock.style.zIndex = CONFIG.CONNECTED_TARGET_ZINDEX;
        ...
        // *** DEBUG Log right before playing sound ***
        if (CONFIG.DEBUG) console.log(`[PerformSnap] Snap appears successful, playing sound...`);
        playSnapSound();
        addSnapEffectAnimation(sourceBlock);
        sourceBlock.draggable = false;

        if (CONFIG.DEBUG > 1) console.log(`[PerformSnap] Snap success details complete.`);
        return true;
    } catch (err) {
        console.error(`[PerformSnap] Error during snap:`, err);
        try { detachBlock(sourceBlock, false); } catch (detachErr) { console.error(`[PerformSnap] Cleanup detach error:`, detachErr); }
        sourceBlock.draggable = true; sourceBlock.style.zIndex = ''; if(targetBlock) targetBlock.style.zIndex = '';
        return false;
    }
  }
  ...
  // ========================================================================
  // ניתוק בלוקים ותפריט (ללא שינוי מ-3.5)
  // ========================================================================
  function showDetachMenu(x, y, block) { /* ... (same as v3.5) ... */
  ...
  function closeMenuOutside(e) { /* ... (same as v3.5) ... */
  ...
  function removeDetachMenu() { /* ... (same as v3.5) ... */
  ...
  function detachBlock(blockToDetach, animate = true) {
    if (!blockToDetach || !blockToDetach.hasAttribute('data-connected-to')) { if (blockToDetach) { blockToDetach.draggable = true; blockToDetach.style.zIndex = ''; } return; }
    ...
    if (!targetId || !direction) { console.warn(`[Detach] Missing data on ${blockToDetach.id}.`); blockToDetach.removeAttribute('data-connected-to'); blockToDetach.removeAttribute('data-connection-direction'); blockToDetach.classList.remove('connected-block'); blockToDetach.draggable = true; blockToDetach.style.zIndex = ''; return; }
    if (CONFIG.DEBUG) console.log(`[Detach] Detaching ${blockToDetach.id} from ${targetId}.`);
    // Cleanup source
    blockToDetach.removeAttribute('data-connected-to'); blockToDetach.removeAttribute('data-connection-direction'); blockToDetach.classList.remove('connected-block'); blockToDetach.draggable = true; blockToDetach.style.zIndex = '';
    ...
    if (targetBlock) {
        ...
        if (!hasOtherConn) { targetBlock.classList.remove('has-connected-block'); targetBlock.style.zIndex = ''; if (CONFIG.DEBUG) console.log(`[Detach] Reset zIndex for target ${targetId}.`); }
        else { if (CONFIG.DEBUG > 1) console.log(`[Detach] Target ${targetId} still has connections.`); }
  ...
  // ========================================================================
  function generateUniqueId(block) { if (block.id) return block.id; const prefix = block.dataset.type || block.classList[0] || 'block'; let id = `${prefix}-${Math.random().toString(36).substring(2, 7)}`; let i = 0; while (document.getElementById(id) && i < 10) { id = `${prefix}-${Math.random().toString(36).substring(2, 7)}-${i++}`; } if (i >= 10) id = `${prefix}-${Date.now()}`; block.id = id; if (CONFIG.DEBUG) console.log(`Generated ID: ${id}`); return id; }
  ...
  function initializeSystem() {
    ...
    if (CONFIG.PLAY_SOUND && CONFIG.USE_EMBEDDED_SOUND) addSoundTestButton(); // Add test button only if using embedded sound
    window.blockLinkageInitialized = true;
    const soundSource = CONFIG.USE_EMBEDDED_SOUND ? 'Embedded Base64' : (CONFIG.SOUND_PATH || 'None configured');
    console.log(`Block linkage system initialized (Version 3.6 - Embedded Sound)`);
    console.log(`Config: Thresh=${CONFIG.CONNECT_THRESHOLD}, Overlap=${CONFIG.VERTICAL_OVERLAP_REQ*100}%, Sound=${CONFIG.PLAY_SOUND ? soundSource : 'Disabled'}, Z-Idx(S/T): ${CONFIG.CONNECTED_SOURCE_ZINDEX}/${CONFIG.CONNECTED_TARGET_ZINDEX}`);
  }
