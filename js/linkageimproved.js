...
(function() {
  ...
  // Styles (Yellow halo, Blue indicator) - No changes from v3.4
  ...
  // Audio Init, Test Button, Play Sound - No changes from v3.4
  ...
  // Listeners, Block Detection, Context Menu, MouseDown - No changes from v3.4
  ...
  // Global Listeners, MouseLeave, MouseMove - No changes from v3.4
  ...
  function handleMouseMove(e) { if(!isDraggingBlock||!currentDraggedBlock)return;e.preventDefault();const pE=document.getElementById('program-blocks');if(!pE){handleMouseUp(e);return;}const pR=pE.getBoundingClientRect();let nL=e.clientX-pR.left-dragOffset.x+pE.scrollLeft;let nT=e.clientY-pR.top-dragOffset.y+pE.scrollTop;const bW=currentDraggedBlock.offsetWidth;const bH=currentDraggedBlock.offsetHeight;const sW=pE.scrollWidth;const sH=pE.scrollHeight;nL=Math.max(0,Math.min(nL,sW-bW));nT=Math.max(0,Math.min(nT,sH-bH));currentDraggedBlock.style.left=Math.round(nL)+'px';currentDraggedBlock.style.top=Math.round(nT)+'px';checkAndHighlightSnapPossibility(); } // Check highlight on move

  // *** MOUSE UP - VERSION 3.5 LOGIC ***
  // Snaps if a potential target was identified during the drag (Jump behavior)
  function handleMouseUp(e) {
    if (!isDraggingBlock || !currentDraggedBlock) return;

    const blockReleased = currentDraggedBlock;
    const candidateTarget = potentialSnapTarget; // Target identified during move
    const candidateDirection = snapDirection;   // Direction identified during move

    if (CONFIG.DEBUG) console.log(`[MouseUp v3.5] Releasing ${blockReleased.id}. Candidate: ${candidateTarget?.id || 'none'}, Dir: ${candidateDirection || 'none'}`);

    // --- Cleanup drag state and visuals ---
    ...
    // *** Snap Decision: Based ONLY on whether a candidate was found during drag ***
    let performSnap = false;
    if (candidateTarget && candidateDirection && document.body.contains(candidateTarget)) {
        // A valid target was highlighted at some point. Decide to snap.
        if (CONFIG.DEBUG) console.log(`[MouseUp v3.5] Candidate ${candidateTarget.id} was highlighted. Attempting snap (will jump if needed).`);
        performSnap = true;
    } else {
        if (CONFIG.DEBUG) console.log(`[MouseUp v3.5] No valid candidate identified during drag. No snap attempt.`);
    }

    // Execute snap if decided
    if (performSnap) {
      // performBlockSnap calculates the final position and applies it ("the jump")
      const snapSuccess = performBlockSnap(blockReleased, candidateTarget, candidateDirection);
      if (!snapSuccess) {
          // Snap failed technically (e.g., target removed), make draggable again
          blockReleased.draggable = true;
          if (CONFIG.DEBUG) console.log(`[MouseUp v3.5] Snap attempt failed (performBlockSnap=false). ${blockReleased.id} remains draggable.`);
      } else {
           // Snap succeeded, block is now connected and not draggable
           if (CONFIG.DEBUG) console.log(`[MouseUp v3.5] Snap successful. ${blockReleased.id} jumped to position.`);
      }
    } else {
      // No snap occurred, block stays where it was released
      if (CONFIG.DEBUG) console.log(`[MouseUp v3.5] No snap performed. ${blockReleased.id} remains free.`);
      blockReleased.draggable = true; // Ensure free block is draggable
    }

    if (CONFIG.DEBUG) console.log(`[MouseUp v3.5] ----- End MouseUp for ${blockReleased.id} -----`);
  }

  // Check Highlight (MouseMove) - No changes from v3.4
  function checkAndHighlightSnapPossibility() {
    ...
    let minDistance = CONFIG.CONNECT_THRESHOLD + 1; // Using threshold 8

    document.querySelectorAll('.snap-target').forEach(el => { el.classList.remove('snap-target', 'snap-left', 'snap-right'); });
    potentialSnapTarget = null; snapDirection = null; // Reset candidate
    removeFuturePositionIndicator();
    ...
    if (bestTarget) {
      if (CONFIG.DEBUG > 1) console.log(`[Highlight] Threshold met (${CONFIG.CONNECT_THRESHOLD}px): ${currentDraggedBlock.id} -> ${bestTarget.id}. Activating visuals.`);
  ...
  // Calculate Snap Info (Distance/Overlap) - No changes from v3.4
  ...
  // Perform Snap (The actual "Jump") - No changes from v3.4
  function performBlockSnap(sourceBlock, targetBlock, direction) {
    ...
    try {
      ...
      // *** This is the "jump" - setting the final position ***
      ...
      sourceBlock.draggable = false; // Connected block is not draggable by default
  ...
  // Indicator Update/Remove, Detach functions, Animation, Unique ID - No changes from v3.4
  ...
  function initializeSystem() {
    const initFlag = 'blockLinkageInitialized_v3_5';
    if (window[initFlag]) { if (CONFIG.DEBUG) console.log("Block linkage v3.5 already initialized."); return; }
