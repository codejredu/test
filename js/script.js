...
function createBlockElement(block, category) {
    ...
    blockImage.draggable = false; // למנוע גרירת תמונה
    ...
    blockContainer.addEventListener("dragstart", (event) => {
        try {
            const dataToSend = JSON.stringify({
                ...
                category: category,
                name: block.name,
                svgFile: block.svgFile // חשוב להעביר את כל הנתונים
            });
            event.dataTransfer.setData("text/plain", dataToSend);
            event.dataTransfer.effectAllowed = "copy"; // עותק חדש נוצר באזור התכנות
        } catch (e) {
...
function populateBlockPalette(category) {
    ...
    if (!categoryDiv) {
        console.error(`Category div not found for ${category}`); return;
    }
    categoryDiv.innerHTML = ""; // ניקוי

    if (!blocks[category] || blocks[category].length === 0) {
        console.warn(`No blocks defined for category ${category}`); return;
    }
    // console.log(`Populating category: ${category} with ${blocks[category].length} blocks`); // לוג פחות קריטי

    blocks[category].forEach(block => {
        // console.log(`Creating block: ${block.name} (${block.svgFile})`); // לוג פחות קריטי
...
function handleCategoryChange(category) {
    // console.log(`Changing category to: ${category}`); // לוג פחות קריטי
    ...
    else { console.warn(`Tab not found for category: ${category}`); }

    if (categoryDiv) {
        categoryDiv.classList.add("active");
        const blockPalette = document.getElementById("block-palette");
        if (blockPalette) {
            blockPalette.style.borderColor = getCategoryColor(category);
        }
        populateBlockPalette(category);
    } else { console.warn(`Block category container not found for: ${category}`); }
}
...
function getCategoryColor(category) {
    ...
    } catch (e) { console.error("Error getting category color:", e); return '#CCCCCC'; }
}

// ========================================================================
// פונקציה לטיפול בהשמטת בלוק מהפלטה לאזור התכנות
// ========================================================================
function handleDrop(event) {
    ...
    try { data = JSON.parse(dataString); }
        ...
        // אם זה לא JSON מהפלטה, נניח שזה גרירה פנימית שמטופלת ע"י linkageimproved.js
        // console.warn("Ignoring drop without valid palette data.");
        return;
    }

    if (!data || !data.type || !data.category || !data.name || !data.svgFile) {
        console.warn("Dropped data missing required fields. Ignoring.", data); return;
    }
    ...
    try {
        ...
        // יצירת אלמנט הבלוק *לאזור התכנות* (בלי מאזין dragstart)
        ...
        // רישום הבלוק החדש במערכת ההצמדה
...
function createProgrammingBlockElement(block, category) {
    ...
    blockImage.onerror = function() {
        console.warn(`(Prog Area) SVG image not found: assets/block/${block.svgFile}`);
        blockContainer.textContent = block.name;
        blockContainer.style.border = "1px dashed orange";
        blockContainer.style.backgroundColor = "#fff5e6";
        blockContainer.style.padding = "5px";
        blockContainer.style.display = 'inline-block';
        blockContainer.style.minWidth = '50px';
    };
    blockContainer.appendChild(blockImage);

    // *** אין כאן draggable=true או addEventListener('dragstart') ***
    // הגרירה מטופלת ע"י linkageimproved.js
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (programmingArea) {
        programmingArea.addEventListener("dragover", (event) => {
            if (event.dataTransfer.types.includes("text/plain")) { // אפשר drop רק לנתונים מהפלטה
                event.preventDefault();
        ...
        console.log("Programming area drag listeners initialized (for palette drops)");
    } else { console.error("Programming area element not found!"); }
    ...
    } else { console.error("No category tabs found!"); }
    ...
    if (clearAllButton && programmingArea) {
        clearAllButton.addEventListener("click", () => { programmingArea.innerHTML = ""; console.log("Programming area cleared"); });
    }

    // אתחול גרירת דמות (קוד זה נשאר ללא שינוי)
    const character = document.getElementById('character');
    const stageElement = document.getElementById('stage');
    if (character && stageElement) { /* ... קוד גרירת דמות ... */
         ...
         let isCharDragging = false; let charOffsetX, charOffsetY;
    ...
    if (activeTab && activeTab.getAttribute('data-category')) {
        ...
        // אם כבר יש טאב אקטיבי (אולי מ-HTML), עדכן את הצבעים והבלוקים
        ...
        if (triggeringTab) {
            handleCategoryChange(initialCategory); // הפעל קטגוריה ראשונית
             ...
             if (firstTab) { initialCategory = firstTab.getAttribute('data-category'); handleCategoryChange(initialCategory); }
             else { console.warn("No category tabs found to initialize."); }
        }
    }
