...
// פונקציה ליצירת בלוק גרפי - עודכנה להוספת תמונות SVG והוספת ריבוע ועיגול
...
if (blockIndex) { // אם קיים block-index, זה אומר שגוררים בלוק בתוך אזור התכנות
    ...
    const draggedBlock = programmingArea.children.item(draggedBlockIndex); // השתמש ב-item כדי למנוע בעיות עם אוספים חיים
...
const newTab = document.querySelector(`.category-tab` + `[` + `data-category="${category}"]`);
newTab?.classList.add("active");

blockCategories.forEach(catDiv => catDiv.classList.remove("active"));
const newCategoryDiv = document.getElementById(`${category}-blocks`);
newCategoryDiv?.classList.add("active");
